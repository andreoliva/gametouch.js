...
var gamepad_config = {
  ...
  // bindB: { press: keyDown, release: null, keyCode: 33 },
  bindDirections: {
...
function initGamepad(gp, conf){
  ...
  function analogStart(evt){
    gp.stick = {
      pos: {
        x: controls.analog_left.ball.getBoundingClientRect().left/gp.scale,
        y: controls.analog_left.ball.getBoundingClientRect().top/gp.scale
      },
      radius: controls.analog_left.ball.offsetWidth/2,
  ...
  function analogMove(evt){
    ...
    animateStick(controls.analog_left.ball, x, y);
  ...
  function analogEnd(evt){
    ...
    setCSSTransform(controls.analog_left.ball, 'translate(0px, 0px)');
  }
  ...
  function generateAnalog(ctrls, side){
    ...
    d[0].className = 'gamepad-el gamepad-analog-arrows gamepad-analog-up';
    d[1] = document.createElement('div');
    d[1].className = 'gamepad-el gamepad-analog-arrows gamepad-analog-down';
    d[2] = document.createElement('div');
    d[2].className = 'gamepad-el gamepad-analog-arrows gamepad-analog-left';
    d[3] = document.createElement('div');
    d[3].className = 'gamepad-el gamepad-analog-arrows gamepad-analog-right';
    for (var i = 0; i < d.length; i++) controls['analog_'+side].appendChild(d[i]);
    controls['analog_'+side].appendChild(ball);
    controls['analog_'+side].ball = ball;
  }
